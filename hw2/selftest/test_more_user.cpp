...
void mydata::exploid()
{
	...
	int tmpi=-1,tmpt=-1;
	for(int i=0;i<M;++i)
		if( tmpt != vrowdata[i].time)
			vtime.push_back( tmpt = vrowdata[i].time);
	for(int i=0;i<M;++i)
		if( tmpi != sorti[i]->item)
			vitem.push_back( tmpi = sorti[i]->item);
	...
	{
		++count;
		int i1 = (int)floor( (double)vitem.size()*rand()/RAND_MAX);
		int i2 = (int)floor( (double)vitem.size()*rand()/RAND_MAX);
		if(i1==vitem.size() ) --i1;
		if(i2==vitem.size() ) --i2;

		int t1 = (int)floor( (double)vtime.size()*rand()/RAND_MAX);
		int t2 = (int)floor( (double)vtime.size()*rand()/RAND_MAX);
